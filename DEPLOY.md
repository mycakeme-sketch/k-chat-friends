...
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase **Settings → API → service_role** (서버 전용, Git·클라이언트에 넣지 않음). **`/dev/characters`에서 DB 저장**할 때 필요 |
| `CHARACTER_ADMIN_SECRET` | 임의의 긴 문자열(관리자 비밀번호). **`/dev/characters` 저장/시드** 시 입력값과 같아야 함 |
